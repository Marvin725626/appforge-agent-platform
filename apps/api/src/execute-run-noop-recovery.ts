...
export function createNoopRecoveryRequest(currentRequest: string): string {
    ...
    return [
        ...
        "This recovery is a patch task, not a planning task: locate the exact affected source/CSS/asset first, then apply the smallest real diff that satisfies the newest request.",
        "Do not return finish until the newest request has produced a real user-visible source-file or asset change.",
        "If the user asked to shorten, recolor, resize, move, hide, delete, or fix one visible region, modify that exact region instead of regenerating unrelated page structure.",
        "If an oldText edit fails or no selector is obvious, re-read the current file and retry with a smaller exact snippet from the current contents.",
